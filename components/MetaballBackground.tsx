...
export function MetaballBackground() {
  ...
  useEffect(() => {
    ...
    const pixelRatio = Math.min(devicePixelRatio, isMobile ? 1.0 : 1.25)
    ...
    const material = new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        float rayMarch(vec3 ro, vec3 rd) {
          float t = 0.0;
          int maxSteps = uIsMobile > 0.5 ? 14 : (uIsSafari > 0.5 ? 16 : 32);
    ...
    function getStoryText(x: string, y: string, radius: string, merges: number, fps: number) {
      ...
      return `coordinates: (${x}, ${y}) | field: ${radius}u | merges: ${merges} | temporal flux: ${fps}hz`
    }
    ...
    const handleResize = () => {
      ...
      const currentPixelRatio = Math.min(devicePixelRatio, isMobile ? 1.0 : 1.25)
    ...
    function applyPreset(presetName: string) {
      ...
      const themeColor = presetThemeColors[presetName] || '#5EEAD4'
      ...
      Object.keys(preset).forEach((key) => {
        if (key in settings) {
          ; (settings as any)[key] = preset[key]
        }
      })
    ...
    const render = () => {
      ...
      // If tab/document is hidden, pause rendering completely
  ...
  return (
    <>
      ...
      {storyText && (
        <div className="fixed bottom-4 right-4 z-20 font-mono text-[10px] text-muted-foreground/60 text-right pointer-events-none hidden md:block select-none tracking-wider">
          {storyText}
        </div>
      )}
    </>
